...
- `apps/mobile` contains the Expo app, React Navigation stack, screens, local state provider, AsyncStorage persistence, shared UI components, theme tokens, and the first multiplayer lobby plus active-game UI/client slices.
...
- `packages/game-engine` now contains backend-neutral multiplayer write plans that describe conditional room/event/snapshot/private-hand/idempotency writes for future physical persistence adapters.
...
- Mobile multiplayer active-game shell for rendering the public game snapshot, loading the viewer's private hand, showing table/score/turn state, and submitting pass/numeric bid actions through AppSync.
...
- Multiplayer: engine-level authority, durable record primitives, runtime schemas, write plans, deployed AppSync/Lambda/DynamoDB wiring, mobile auth/network helpers, lobby UI, and the first active-game bidding UI exist, but there is still no trump/trick-play multiplayer UI, mobile subscription handling, reconnect UX, or production lifecycle/abuse handling.
...
- Multiplayer restore validates accepted events before replay, multiplayer actions/storage/reconnect payloads pass through runtime parsers, and write plans validate accepted event streams before persistence writes. Schema migrations and physical backend adapters are still missing.
...
8. Expand mobile multiplayer active-game support beyond bidding with trump selection, trick play, subscription handling, and reconnect UX.
...
- Multiplayer persistence planning is now represented as backend-neutral write plans before any DynamoDB/AppSync adapter exists.
...
- ADR-0002 through ADR-0009 document why M1 is local-first, how server authority should work, why scorekeeper stays separate, why the first multiplayer slice is backend-neutral, why public snapshots are split from private hands, why restore uses validated replay, why multiplayer boundary payloads are parsed at runtime, and why persistence writes are planned behind a backend-neutral boundary before AWS.
